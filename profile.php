...
<body>
    ...
    <?php if (isset($_GET["id"])) : ?>
        <!-- get user from database using $_GET["id"] then put it in a variable called $thatUser-->
        ...
        <div class="profileContainer">
            ...
            <?php
            if (isset($thatUser["photo"])) {
                if ($thatUser["photo"]) {
                    $img = "./uploads/" . $thatUser["photo"];
                } else {
            ...
            ?>
            <h2><?= $thatUser["prenom"] . " " . $thatUser["nom"] ?></h2>
            <h6><?= $thatUser["email"] ?></h6>
            <h4>postes:</h4>
            <?php
            $nopost = true;
            //check $thatUser posts and not your posts here and keep the same logic
            while ($myposts = $result2->fetch_assoc()) {
                $nopost = false;
                echo "<h5><a href='./post.php?id=" . $myposts["id_post"] . "'>" . $myposts["nom_post"] . "</a></h5>"; // change it to $post["id"] and $post["nom"] if it is an array and not an object (i am not sure of the return type)
            }
            if ($nopost) {
                echo "<h5>pas encore de postes..</h5>";
            }
            ?>
        </div>
    <?php else : ?>
        ...
        <div class="profileContainer">
            ...
            <form action="upload_profile.php" method="POST" enctype="multipart/form-data" id="update_form">
                <div class="formGrid">
                    ...
                    <div class="selfGrid"><span id="erreur_nom"></span></div>
                    <div class="selfGrid"></div>
                    <div>
                    ...
                    <div class="selfGrid"><span id="erreur_prenom"></span></div>
                    <div class="selfGrid"></div>
                    <div>
                    ...
                    <div class="selfGrid"><span id="erreur_photo"></span></div>
                    <div class="selfGrid"></div>
